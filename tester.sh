...
FTR_NAME="new_text_features7.rtData"
cd $SCRATCHDIR
N_JOBS=1
...
python $SCRATCHDIR/feature_converter.py $FTR_NAME $SCRATCHDIR/$NUMBER-$NAME $NUMBER
split -l 442239 -a 2 -d $NUMBER-$NAME document_
# ls $SCRATCHDIR
#for m in 0 1 2; do
...
python $SCRATCHDIR/rank-py.py $METRIC $ITER &> $ITER-$METRIC.txt
#NAME=$i-$NAME
#done
#done
